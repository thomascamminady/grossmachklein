...
def to_dataframe(piece_id: int, length: float, type: str, cut_id: int) -> pd.DataFrame:
    return pd.DataFrame(
...
def create_dataframe(
    ...
    for piece_id, cuts_for_piece in enumerate(optimal_cuts):
        ...
        for cut_id, cut_length in enumerate(cuts_for_piece):
            df_list.append(to_dataframe(piece_id + 1, cut_length, "Schnitt", cut_id))

        waste = long_stick - sum(cuts_for_piece)
        df_list.append(to_dataframe(piece_id + 1, waste, "Waste", len(cuts_for_piece)))
